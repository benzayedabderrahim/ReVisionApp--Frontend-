...
const AlgorithmVideos = () => {
    ...
    useEffect(() => {
        const fetchVideos = async () => {
            try {
                setLoading(true);
                setError('');
                const response = await axios.get('http://127.0.0.1:8000/api/algorithm/');
                
                if (response.data && response.data.status === 'success' && response.data.videos) {
                    const sortedVideos = [...response.data.videos].sort((a, b) => {
                        const aPercentage = a.positive_percentage || 0;
                        const bPercentage = b.positive_percentage || 0;
    ...
    const getPositivityPercentage = (video) => {
        return video.positive_percentage || 
               Math.round((video.positive_comments_count / video.comments_count) * 100) || 0;
    };
...
export default AlgorithmVideos;